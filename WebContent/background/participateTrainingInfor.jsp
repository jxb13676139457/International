		        ...
		        <div  style="text-align:center">
                   ...
                   <c:if test="${totalPage>0}">
                         ...
                         <c:if test="${currentPage>1}">
                             [<a href="participateTrainingAction?pageNo=${currentPage-1}">上一页</a>]
                        </c:if>
         
                         <c:if test="${currentPage<totalPage}">
                            [<a href="participateTrainingAction?pageNo=${currentPage+1}">下一页</a>]
                          </c:if>
         
                         [<a href="participateTrainingAction?pageNo=${totalPage}">尾页</a>]	
                                                                                                     第${currentPage}页/共${totalPage}页
                      </c:if>
                      </s:if>
                      <s:else>
                             <c:if test="${totalPage>0}">
                         [<a href="participateTrainingAction!searchInformation?pageNo=1 & temp=1">首页</a>]
         
                         <c:if test="${currentPage>1}">
                             [<a href="participateTrainingAction!searchInformation?pageNo=${currentPage-1} & temp=1">上一页</a>]
                        </c:if>
         
                         <c:if test="${currentPage<totalPage}">
                            [<a href="participateTrainingAction!searchInformation?pageNo=${currentPage+1} & temp=1">下一页</a>]
                          </c:if>
                      ...
                      <!-- 分页 -->
			  		[<a href="attendTrainingAction!showAttend?pageNo=1&loginUserName=${sessionScope.searchAttendTraining}">首页</a>]
						<c:choose>
							<c:when test="${currentPage>1}">
								[<a href="attendTrainingAction!showAttend?pageNo=${currentPage-1}&loginUserName=${sessionScope.searchAttendTraining}">上一页</a>]
							</c:when>
						</c:choose>
						<c:choose>
							<c:when test="${currentPage<totalPage}">
								[<a href="attendTrainingAction!showAttend?pageNo=${currentPage+1}&loginUserName=${sessionScope.searchAttendTraining}">下一页</a>]
							</c:when>
						</c:choose>
					[<a href="attendTrainingAction!showAttend?pageNo=${totalPage}&loginUserName=${sessionScope.searchAttendTraining}">尾页</a>]
					第${currentPage}页/共${totalPage}页
                  </div>
              </div>
