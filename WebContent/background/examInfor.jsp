		  ...
		  <div class="row">
			...
			<div class="row" style="margin:5px">
                ...
                <ul class="nav top-menu">                    
                    <li>
                       <form class="navbar-form" action="simulationExamAction!searchInformationbyTime" method="post">
                          <div style="color:#000000">
                          <label>请输入模拟考试时间：</label>
                           <sx:datetimepicker name="searchSimulationTime" displayFormat="yyyy-MM-dd"/>
                            ...
                            <a href="addExam.jsp"><i class="icon_plus_alt2"></i>&nbsp;&nbsp;<b>添加</b></a></button>
                              <button type="button" class="btn btn-default" style="width:120px;height:30px">
                            <a href="simulationExamAction"><i class="icon_menu"></i>&nbsp;&nbsp;<b>显示全部</b></a></button>
                          
                             
                            <button type="button" class="btn btn-default" style="width:80px;height:30px">
                            <a href="exportSimulationExamExcel?searchName=${searchSimulationTime}"><i class="icon_upload"></i>&nbsp;&nbsp;<b>导出</b></a></button>
                         </div>
                        </form>
                    </li>                    
                </ul>
                                 ...
                                 <td>
                                  <div class="btn-group">
                                     <!-- <a class="btn btn-primary" href="#"><i class="icon_plus_alt2"></i></a> --> 
                                      <a class="btn btn-default" href="javascript:searchProcess('<s:property value="#user.id"/>','simulationExamAction!searchObjectById?id=')"><i class="icon_pencil"></i></a>
                                      <a class="btn btn-default" href="javascript:deleteProcess('<s:property value="#user.id"/>', 'simulationExamAction!deleteObject?id=')"><i class="icon_trash_alt"></i></a>
		       ...
		       <div  style="text-align:center">
		          <s:if test="#status==1"> 
                   <c:if test="${totalPage>0}">
                         [<a href="simulationExamAction?pageNo=1">首页</a>]
         
                         <c:if test="${currentPage>1}">
                             [<a href="simulationExamAction?pageNo=${currentPage-1}">上一页</a>]
                        </c:if>
         
                         <c:if test="${currentPage<totalPage}">
                            [<a href="simulationExamAction?pageNo=${currentPage+1}">下一页</a>]
                          </c:if>
         
                         [<a href="simulationExamAction?pageNo=${totalPage}">尾页</a>]	
                                                                                                     第${currentPage}页/共${totalPage}页
                      </c:if>
                      </s:if>
                      <s:else>
                        <c:if test="${totalPage>0}">
                         [<a href="simulationExamAction!searchInformationbyTime?pageNo=1  & temp=1">首页</a>]
         
                         <c:if test="${currentPage>1}">
                             [<a href="simulationExamAction!searchInformationbyTime?pageNo=${currentPage-1}  & temp=1">上一页</a>]
                        </c:if>
         
                         <c:if test="${currentPage<totalPage}">
                            [<a href="simulationExamAction!searchInformationbyTime?pageNo=${currentPage+1}  & temp=1">下一页</a>]
                          </c:if>
         
                         [<a href="simulationExamAction!searchInformationbyTime?pageNo=${totalPage}  & temp=1">尾页</a>]	
                                                                                                     第${currentPage}页/共${totalPage}页
                      </c:if>
                      </s:else>
                  </div>
